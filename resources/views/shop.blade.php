                    ...
                    <div>
                        <div class="product-box">
                            <div class="img-wrapper">
                                <div class="front">
                                    <a href="{{route('shop.produit.details',['slug'=>$produit->slug])}}">
                                        <img src="assets/images/fashion/product/front/{{$produit->image}}"
                                            class="bg-img blur-up lazyload" alt="">
                                    </a>
                                </div>
                                <div class="back">
                                    <a href="{{route('shop.produit.details',['slug'=>$produit->slug])}}">
                                        <img src="assets/images/fashion/product/back/{{$produit->image}}"
                                            class="bg-img blur-up lazyload" alt="">
                                    </a>
                                </div>
                                <div class="cart-wrap">
                                    <ul>
                                        ...
                                        id="cartEffect" class="btn btn-solid hover-solid btn-animation">
                                            ...
                                            <form id="addtocart" method="post" action="{{route('card.store')}}">
                                               @csrf
                                    ...
                                    <h3 class=" text-dark" >
                                        @if ($produit->sale_price) 
                                            {{-- {{ $produit->sale_price }} TND   --}}
                                                  
                                            {{-- <del>{{ $produit->regular_price }} TND </del> <span>  --}}
                                               
                                                 {{ round((($produit->regular_price - $produit->sale_price) / $produit->regular_price) * 100) }}
                                                % off </samp> 
                                        @else
                                           00 % off 
                                        @endif  
                            ...
                            <div class="product-details">
                                <div class="rating-details">
                                    @if ($produit->Categori)
                                        <span class="font-light grid-content">{{ $produit->Categori->name }}</span>
                                    @else
                                        <span class="font-light grid-content">Category not available</span>
                                    @endif
                                ...
                                <div class="main-price">
                                    <a href="{{route('shop.produit.details',['slug'=>$produit->slug])}}" class="font-default">
                                        <h5 class="ms-0">{{$produit->name}}</h5>
                                    </a>
                                    <div class="listing-content">
                                        @if ($produit->Categori)
                                        <span class="font-light">{{ $produit->Categori->name }}</span>
                                    @endif
                                        <p class="font-light">{{$produit->short_description}}</p>
                                    </div>
                                    {{-- <h3 class="theme-color">{{$produit->regular_price}} TND</h3> --}}

                                    <h3 class=" text-dark"  >
                                        @if ($produit->sale_price) 
                                            <del>{{ $produit->regular_price }} TND</del> 
                                                  <BR>
                                            {{-- <del>{{ $produit->regular_price }} TND </del> <span>  --}}
                                                 {{ round((($produit->regular_price - $produit->sale_price) / $produit->regular_price) * 100) }}
                                                % off  
                                        @else
                                           {{ $produit->regular_price }} TND
...
{{--                                            
                                    ...
                                    <div class="listing-content">
                                        @if ($produit->quantity == 0)
                                            <span class="font-light">Out of Stock </span>
                                        @else
                ...
                {{$produits->links("pagination.default")}}
