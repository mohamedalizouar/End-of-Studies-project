                    ...
                    @foreach ($cartItems as $item)                                          
                        <tr>
                            {{-- <td>
                                ...
                                <a href="{{route('shop.produit.details',['slug'])}}">
                                    <img src="{{asset('assets/images/fashion/product/front')}}/{{$item->mage}}" class="blur-up lazyloaded"
                                        alt="{{$item->name}}">
                                </a>
                                @endif
                            ...
                            <td>
                                ...
                                <a href="{{route('shop.produit.details',['slug'])}}">{{$item->name}}</a>
                                {{-- @else
                            ...
                            <td>
                                <h2 class="td-color">{{$item->subtotal()}} TND</h2>
                            </td>
                            <td>
                            ...
                            <div class="cart-box-details">
                                <div class="total-details">
                                    <div class="top-details">
                                        <h3>Cart Totals</h3>
                                        <h6>Sub Total <span>{{cart::instance('card')->subtotal()}} TND</span></h6>
                                        <h6>Tax <span> {{cart::instance('card')->tax()}} TND</span></h6>

                                        <h6>Total <span>{{cart::instance('card')->total()}} TND</span></h6>
                                    </div>
...
<form id="updateCartQty" action="{{route('card.update')}}" method="POST">
@csrf
@method('put')
...
<form id="deleteFromCart" action="{{route('card.remove')}}" method="POST">
@csrf
@method('delete')
<input type="hidden" id="rowId_D" name="rowId" />
</form>

<form id="clearCart" action="{{route('card.clear')}}" method="POST">
@csrf
@method('delete')
</form>
